...
def _load_cuda_libs() -> None:
    ...
    if platform.system() == "Windows":
        for package, folder, pattern in WINDOWS_CUDA_LIBS:
            for lib in sorted((base / package / folder).glob(pattern)):
                try:
    ...
    for package, folder, pattern in LINUX_CUDA_LIBS:
        for lib in sorted((base / package / folder).glob(pattern)):
            try:
